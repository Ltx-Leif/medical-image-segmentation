...
class MambaVisionMixer(nn.Module):
    def __init__(
        self,
        d_model,
        num_heads=4,  # 添加num_heads参数
        **kwargs  # 忽略其他未使用参数
    ):
        super().__init__()
        self.d_model = d_model
        # 使用Transformer块替代Mamba块
...
class MambaVision(nn.Module):
    ...
    def __init__(self,
        ...
        dpr_decoder = [x.item() for x in torch.linspace(0, drop_path_rate, sum(depths))][
                      ::-1]  # encoder 和 decoder dpr 相反
        ...
        for i in range(len(depths)):
            ...
            level = MambaVisionLayer(dim=int(dim * 2 ** i),
                                     ...
                                     drop_path=dpr[sum(depths[:i]):sum(depths[:i + 1])],  # 选择每个layer 的 dpr 参数
                                     downsample=(i < 3),
                                     layer_scale=layer_scale,
    ...
    def forward_features(self, x):
        ...
        # level 1: [2,80,56,56]          level 2: [2,160,28,28]
        # level 3: [2,320,14,14]         level 4: [2,640,7,7]
        for level in self.levels:
            x = level(x)
        x = self.norm(x)  # [2,640,7,7]
        x = self.avgpool(x)  # [2,640,1,1]
        x = torch.flatten(x, 1)  # [2,640]
        return x

    def forward(self, x):
        x = self.forward_features(x)  # [2,640]
        x = self.head(x)  # # [2, 1000]
        return x
...
if __name__ == "__main__":
    pretrained_path = "/root/workspace/code/MambaVision/mambavision/pretrained/mambavision_tiny_1k.pth.tar"
    model = mamba_vision_T(model_path=pretrained_path).cuda()
